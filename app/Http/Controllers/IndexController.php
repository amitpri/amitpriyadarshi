    ...
    public function index()
    {
        ...
        $categories = Category::get(['id', 'name', ]);
         ...
         $blogs =  DB::select("SELECT  a.`id`, a.`name`, a.`summary`, a.`user_id`  , b.`name` as author 
                                        , a.`category_id` , c.`name` as category_name , a.`image1`, a.`image2`, a.`image3`  , a.`top` , a.`top2`, a.`top3` , a.`featured` , a.`popular` 
        ...
        $blogs_popular = $blogs;
         ...
         $bigdata_blogs =  DB::select("SELECT  a.`id`, a.`name`, a.`summary`, a.`user_id`  , b.`name` as author , a.`category_id` , c.`name` as category_name , a.`image1`, a.`image2`, a.`image3` , a.`top` , a.`top2`, a.`top3` , a.`featured` , a.`popular`  
                                       FROM `blogs` a , `users` b,  `categories` c
                                        WHERE  a.`user_id` = b.`id`
                                        AND a.`category_id` = c.`id`
                                        AND c.`name` = 'Big Data' 
        ...
        $bigdata_blogs_top = $bigdata_blogs;
         ...
         $cloud_blogs =  DB::select("SELECT  a.`id`, a.`name`, a.`summary`, a.`user_id`  , b.`name` as author , a.`category_id` , c.`name` as category_name, a.`image1`, a.`image2`, a.`image3`  , a.`top`  , a.`top2`, a.`top3` , a.`featured` , a.`popular` 
                                       FROM `blogs` a , `users` b,  `categories` c
                                        WHERE  a.`user_id` = b.`id`
                                        AND a.`category_id` = c.`id`
                                        AND c.`name` = 'Cloud' 
        ...
        $cloud_blogs_top = $cloud_blogs;
         ...
         $dwh_blogs =  DB::select("SELECT  a.`id`, a.`name`, a.`summary`, a.`user_id`  , b.`name` as author , a.`category_id` , c.`name` as category_name , a.`image1`, a.`image2`, a.`image3` , a.`top`   , a.`top2`, a.`top3` , a.`featured` , a.`popular` 
                                       FROM `blogs` a , `users` b,  `categories` c
                                        WHERE  a.`user_id` = b.`id`
                                        AND a.`category_id` = c.`id`
                                        AND c.`name` = 'DWH'
